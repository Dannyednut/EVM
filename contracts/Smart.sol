...
contract Smart is Reserves, Ownable {
    ...
    function verify(address token) internal view returns (bool result) {
    ...
    // Attempt to call balanceOf and check the return data
    (bool success, bytes memory data) = token.staticcall(
        abi.encodeWithSignature("totalSupply()", msg.sender)
    );

    // Check if the call succeeded and returned valid data (uint256)
    if (success && data.length == 32) {
        // Optionally, you can also check for other ERC20 functions
        ...
        result = false; // Call failed or invalid return data
    }
}

    function addBaseToken(address token) external onlyOwner {
        require((verify(token) == true), 'Not a valid ERC20');
        ...
        )
    {
        ...
        // Dynamic base token detection
        baseToken = (IERC20(pool0Token0).balanceOf(pool0) > IERC20(pool0Token1).balanceOf(pool0)) ? pool0Token0 : pool0Token1;
        quoteToken = (baseToken == pool0Token0) ? pool0Token1 : pool0Token0;
        baseSmaller = (baseToken == pool0Token0);
    }
    ...
    function trade(address pool0, address pool1) external {
    ...
    function uniswapV2Call(
        ...
        if (info.debtToken != WETH){
            baseTokens.add(info.debtToken);
        }
    ...
    function calcBorrowAmount(OrderedReserves memory reserves) internal pure returns (uint256 amount) {
        ...
        // 0 < x < b1 and 0 < x < b2
        bool validSolution = (x1 > 0 && x1 < b1 && x1 < b2) || (x2 > 0 && x2 < b1 && x2 < b2);
        if (validSolution){
            amount = (x1 > 0 && x1 < b1 && x1 < b2) ? uint256(x1) * d : uint256(x2) * d;
        }else{
            uint256 amountBack= fallbackResult(a1, b1, a2, b2);
            amount = amountBack * d;
        }
    }
    ...
    function fallbackResult(int256 a1, int256 b1, int256 a2, int256 b2) internal pure returns(uint256 amt){
        ...
        uint256 pool0Price = uint256(a1/b1);
            uint256 pool1Price = uint256(a2/b2);

            (uint256 maxBorrow, uint256 swapPrice) = pool0Price < pool1Price ? (uint256(b1), pool1Price) : (uint256(b2), pool0Price);
            uint256 maxSwap = pool0Price > pool1Price ? uint256(a1) : uint256(a2);
