...
contract FlashBot is Ownable {
    event LogProfit(string message, uint256 num);
    ...
    function addBaseToken(address token) internal onlyOwner {
        baseTokens.add(token);
        emit BaseTokenAdded(token);
    }

    function removeBaseToken(address token) internal onlyOwner {
        ...
        )
    {
        ...
        if (!baseTokensContains(pool0Token0)) {
            addBaseToken(pool0Token0);
        }
        if (!baseTokensContains(pool0Token1)) {
            addBaseToken(pool0Token1);
        }
    ...
    function flashArbitrage(address pool0, address pool1) external {
        ...
        removeBaseToken(info.baseToken);
        removeBaseToken(info.quoteToken);
