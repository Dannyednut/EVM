...
contract ArbExec is Ownable, ReentrancyGuard {
     ...
     */
    function executeBatch(bytes[] calldata calls) external onlyOwner returns (uint256 successful, uint256 failed) {
        uint256 len = calls.length;
        successful = 0;

        for (uint256 i = 0; i < len; ) {
            (bool success,) = address(this).call(calls[i]);   // call execute()
    ...
    function execute(ArbData calldata arb, bool forceBalancer, uint256 validUntilBlock) external nonReentrant {
        // Allow calls from owner directly OR from this contract (via executeBatch)
        ...
        if (forceBalancer && BALANCER_VAULT != address(0)) _initiateBalancerFlashloan(arbMem, borrowAmt, payload);
    ...
    function _initiateBalancerFlashloan(ArbData memory arb, uint256 amt, bytes memory payload) internal {
        ...
        try IBalancerVault(BALANCER_VAULT).flashLoan(address(this), a, am, payload) {
            ...
            _initiatePoolFlashswap(arb, arb.amountIn, abi.encode(arb, arb.amountIn, startBalance));
        }
    }
