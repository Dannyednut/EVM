...
library Helper {
    ...
    // Golden ratio conjugate: (√5 - 1) / 2 ≈ 0.618034
    uint256 private constant PHI_INV   = 618034;
    uint256 private constant PRECISION = 1000000;

    // Probe amounts (token-agnostic, scaled in wei)
    function getProbes() internal pure returns (uint256[5] memory) {
        return [
            uint256(1e15),
            uint256(1e17),
            uint256(1e19),
    ...
    // uint256[5] private constant PROBES = [1e15, 1e17, 1e19, 1e21, 1e23];

    /// @notice Finds the input amount that maximizes profit for a given path
    /// @param quoter QuoterV2 address
    /// @param tokens Token path (must start and end with same token for cyclic arb)
    /// @param fees Fee tiers for each hop
    /// @param maxIterations Maximum Quoter calls (recommended: 35-40)
    /// @return bestAmount Optimal input amount
    /// @return bestProfit Maximum profit (in tokenIn decimals)
    function findOptimalAmount(
        ...
        (uint256 low, uint256 high, uint256 seedBestAmt, uint256 seedBestProfit) = 
            _findBracket(quoter, tokens, pools, fees);
        ...
        (uint256 resInP0,  uint256 resOutP0)  = _getOrderedReserves(pools[0], tokens[0]);
        if (mode == 1) bestAmount = getAmountOutV2WithFee(bestAmount, resInP0, resOutP0, fees[0]);
    ...
    function _findBracket(
        ...
        bestProfit = 0;
        uint256[5] memory probes = getProbes();
        ...
        high = bestIdx < probes.length - 1 ? probes[bestIdx + 1] : bestAmt * 10;

        return (low, high, bestAmt, bestProfit);
    }

    /// @dev Simulates full path profit using Quoter (same logic as your Python `quote_profit`)
    function _quoteProfit(
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            if (_isUniswapV3(pools[i])) {  // adjust based on your Helper
                // Use V3 Quoter
                ...
                // V2 simulation using your Helper
                (uint112 r0, uint112 r1,) = IUniswapV2Pair(pools[i]).getReserves(); // assuming pool address
                address t0 = _token0(pools[i]); // adjust as needed
                current = tokens[i] == t0 
        ...
        // For cyclic arb: profit = final output - input
